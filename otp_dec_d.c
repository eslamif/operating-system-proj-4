...
int main(int argc, char *argv[])
{
        ...
        FILE *fp_log = NULL;
        fp_log = fopen("log_otp_dec_d.txt", "w+");
        if(fp_log == NULL) {
		...
		*/
        ...
        while(1) {
		        ...
		        /****** ENCODE PLAINTEXT TO CIPHERTEXT ******/
				//uncomment printf from line 308 to 391 for debugging
		        char alpha_vals[] = "a bcdefghijklmnopqrstuvwxyz";
		
		        //Obtain int values of ciphertext
		        ...
		        for (i = 0; i < cipher_size; i++) {
		                for (j = 0; j < sizeof(alpha_vals); j++) {
		                        if (cipher_mod[i] == j) {
		                                ciphertext[i] = toupper(alpha_vals[j]);
		                        }
		                }
		        }
		        //Print ciphertext
